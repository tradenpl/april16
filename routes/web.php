...
Route::get('verify/account/{confirmation_code}', [HomeController::class, 'getVerifyAccount'])->where('confirmation_code','[A-Za-z0-9]+');
 ...
 Route::group(['middleware' => 'auth'], function() {
	Route::post('/khalti/payment/verify',[AddFundsController::class,'verifyKhaltiPayment'])->name('khalti.verifyPayment');
	Route::get('/khalti/payment',function(){
		return view('khalti');
	})->name('addfund.khalti');
	...
	Route::post('/khalti/subscription/store',[SubscriptionsController::class,'storekhaltiPayment'])->name('subscription.khalti.store');
   ...
   Route::post('/khalti/payment/store',[AddFundsController::class,'storekhaltiPayment'])->name('addfund.khalti.store');
	...
	Route::post('/tip/khalti-payment/store',[TipController::class,'storekhaltiPayment'])->name('tip.khalti.store');
	
	// Pay Per Views
	Route::post('send/ppv', [PayPerViewController::class, 'send'] );
	Route::post('/ppv/khalti-payment/store',[PayPerViewController::class,'storekhaltiPayment'])->name('ppv.khalti.store');
