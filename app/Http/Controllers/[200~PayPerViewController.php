...
class PayPerViewController extends Controller
{
  ...
  $validator = Validator::make($this->request->all(), [
    ...
    switch ($this->request->payment_gateway_ppv) {
      case 'wallet':
        ...
        case 12;
        return $this->sendkhalti($this->request->amount,'ppv.khalti.store',$this->request->id,$this->request->isMessage?$this->request->isMessage:null);
        break;
  ...
  protected function storekhaltipayment(Request $request)
  {
  
      // Insert DB
      //========== Processor Fees
      $amount = $request->amount;
      $payment = PaymentGateways::whereName('khalti')->first();

      // Check if it is a Message or Post
      $media = $request->isMessage ? Messages::find($request->message_id) : Updates::find($request->message_id);

      // Admin and user earnings calculation
      $earnings = $this->earningsAdminUser($media->user()->custom_fee, $request->amount, $payment->fee, $payment->fee_cents);
      ...
      $media->user()->increment('balance', $earnings['user']);
       ...
       if ($request->isMessage) {
         // $user_id, $updates_id, $messages_id
         $this->payPerViews(auth()->user()->id, 0, $request->message_id);
         $url = url('messages', $media->user()->id);
         ...
         // Send Notification - destination, author, type, target
         Notifications::send($media->user()->id, auth()->user()->id, '6', $request->message_id);

         // Get message to show live
         $message = Messages::whereId($request->message_id)->get();
         ...
         // $user_id, $updates_id, $messages_id
         $this->payPerViews(auth()->user()->id, $request->message_id, 0);
         $url = url($media->user()->username, 'post').'/'.$request->message_id;
         ...
         // Send Notification - destination, author, type, target
         Notifications::send($media->user()->id, auth()->user()->id, '7', $request->message_id);
       }
