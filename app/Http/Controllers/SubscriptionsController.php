...
$plan = $creator->plans()
 ->whereInterval($this->request->interval)
    ->firstOrFail();
        ...
        if($payment->name=='khalti'){
          return $this->sendkhalti($plan->price,'subscription.khalti.store',$this->request->id,$this->request->interval);
        }
  ...
  /**
   ...
   public function storekhaltiPayment(Request $request) {
// Find user
$creator = User::whereId($request->message_id)
->whereVerifiedId('yes')
  ->firstOrFail();

// Check if Plan exists
$plan = $creator->plans()
->whereInterval($request->isMessage)
 ->firstOrFail();
